...
first = 0.0;         % start point
second = -.3;         % second start point (if needed)
error = 10e-10;

syms x;
g0 = (x + x*exp(-x) + cos(x))^2;
g1 = diff(g0);
g2 = diff(g1);

g = matlabFunction(g0);
g_p = matlabFunction(g1);
g_dp = matlabFunction(g2);

next_newton = @(cur) (cur - (g(cur) / g_p(cur)));
next_secant = @(cur,last) (cur - g(cur)*(cur-last) / (g(cur)-g(last)) );

modded_u = @(x) (g(x) / g_p(x));
modded_uprime = @(x) ((1 - (g(x) .* g_dp(x)) / g_p(x)^2));
...
next_modded_newton = @(cur) (cur - ( g(cur)*g_p(cur) ) / ( (g_p(cur)^2) - g_dp(cur)*g(cur)) );
...
i = 1;
newton_error(1) = 10;
while (i < (maxlen-1) && newton_error(i) > error)
...
j = 2;
while (j < (maxlen-1) && secant_error(j) > error)
...
Modded_Newton(1) = g(first);
k = 1;
modded_error(1) = 10;
while (k < (maxlen-1) && modded_error(k) > error)
...
figure('name','Question 2, part 2')
subplot(1,2,1)
plot(1:i,Newton(1:i),'or', 1:j,Secant(1:j),'xb', 1:k,Modded_Newton(1:k),'.k')
title('Root Approximation')
xlabel('n')
ylabel('Root Value')
legend('Newton','Secant','Modfied Newton','Location','southeast')

subplot(1,2,2)
semilogy(2:i,abs(Newton(2:i)-Newton_root),'r', ...
    2:j,abs(Secant(2:j)-Secant_root),'b', ...
