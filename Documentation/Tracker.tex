...
\begin{itemize}
\item \{Receiving queues:} 
\item \{Get peer current state} if peer needs to let tracker know its state
\item \{Peer updated file} if a peer updated a file, this queue contains the filesystem difference
\item \{New peer} a peer has joined the network
\item \{Peer left} a peer has logged out of the network
\item \{Peer got file chunk} peer acquired a chunk, need to update the file table
\item \{Peer request for master file system} peer needs the master file system

\item \{Sending queues:}
\item \{Peer transaction update} if a misc message needs to get to peer
\item \{Peer acquired file chunk} Let all peers know to update their file table
\item \{Send peer file system} send a file system difference to a peer
\item \{Broadcast new peer} let everyone know to update their peer table
\item \{Broadcast peer left} let everyone know to update their peer table
\item \{Send master filesystem} send the master filesystem to a new peer
\item \{Send master file table} send the master file table to a new peer
\end{itemize}
